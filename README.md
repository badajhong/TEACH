...
- `algo.critic_state_encoder_dim` and `algo.critic_action_encoder_dim` (both default `256`) balance the inputs to the FlashSAC critic. Each Q head maps state with `BatchNorm -> unit linear` and action with a unit linear projection, then concatenates the branch features. There is no branch-output BatchNorm or ReLU; the original FlashSAC embedder applies `BatchNorm -> unit linear` immediately after concatenation and the rest of its critic is unchanged. The defaults change the 1187-state/23-action fusion into 256 state features plus 256 action features; set both options to `null` to restore the upstream critic. Checkpoints record both dimensions and the encoder architecture; legacy `BatchNorm -> unit linear -> BatchNorm -> ReLU` branch checkpoints are not compatible.
